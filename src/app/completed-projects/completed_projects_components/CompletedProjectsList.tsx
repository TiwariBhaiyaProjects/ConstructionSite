"use client";
// RESPONSIBILITY: Renders the grid of all completed project cards.

import { useState } from "react";
import Image from "next/image";
import { motion } from "framer-motion";
import { MapPin, Ruler, Layers, Calendar } from "lucide-react";
      ...
      <div className="container-custom max-w-7xl mx-auto">
        <Tabs value={activeCategory} onValueChange={setActiveCategory} className="mb-14">
          <TabsList className="flex flex-wrap h-auto gap-3 bg-transparent justify-center">
            {completedProjectsCategories.map((category) => (
              <TabsTrigger
                key={category.id}
                value={category.id}
              ...
              <Card className="group overflow-hidden border border-gray-100 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_50px_rgba(0,0,0,0.12)] rounded-[2rem] transition-all duration-500 bg-white">
                ...
                <CardContent className="p-8">
                  ...
                  <div className="grid grid-cols-2 gap-y-5 gap-x-4 text-sm mb-8">
                    ...
                    <div>
                      <span className="text-gray-400 block text-[11px] uppercase tracking-wider mb-1 flex items-center gap-1"><Calendar className="w-3 h-3 text-brand-primary" /> Year</span>
                      <span className="font-bold text-brand-dark">{project.year}</span>
                    </div>
                  </div>
